...
SYSTEM_PROMPT = """You are NEXORA's Government Agent — an expert on government schemes, subsidies, documents, welfare portals, Aadhaar, PAN in India.

IMPORTANT: Respond ONLY in this JSON format, no markdown, no extra text:
{
  "recommendation": "main actionable advice here",
  "confidence": 0.85,
  "reason": "why this query is relevant to government",
...
def run(query: str, context: str = "") -> dict:
    ...
    try:
        ...
        return {
            "agent": "Government",
            "icon": "🏛️",
            "recommendation": f"Government Agent error: {str(e)}",
            "confidence": 0.0,
            "reason": "",
            "risk": "",
            "status": "error",
